...
% \item Since logarithm is a strictly increasing function, maximizing the natural logarithm $\ln \equiv \log$ of the likelihood is equivalent to maximizing the likelihood.
% \bi 
% \item The log of a product is equal to the sum of logs, that is, 
% $\log(ab) =\log(a) +\log(b)$, so the product over $n$ terms becomes a sum.
% \item It is usually easier to work with sums rather than products. 
% \ei 
%  \item We call the function $\ell(\bs{\theta}) = \log\{L(\bs{\theta})\}$ the \alert{\textbf{log-likelihood}} function.
% \ei
% \end{frame}
...
%  \begin{center}
%   \includegraphics[width = \linewidth]{figures/05-likelihood_tests}
%  \end{center}
% \end{frame}
% 
% 
